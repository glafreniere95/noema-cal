...
let wasMobileLayout = window.matchMedia("(max-width: 699px)").matches;
...
const WEEKDAYS_EN_MOBILE = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"];
...
function renderCalendar({ animate = false } = {}) {
    ...
    monthLabel.textContent = `${MONTHS_EN[baseMonth.getMonth()]} ${baseMonth.getFullYear()}`;
    root.innerHTML = "";
...
function buildMonthSection(monthDate, showTitle) {
    ...
    if (showTitle) {
        ...
        title.textContent = `${MONTHS_EN[monthIndex]} ${year}`;
...
function renderTimeline(dateKey, dateObj, { direction = 0, animate = true } = {}) {
    ...
    dateLabel.textContent = `${dateObj.getDate()} ${MONTHS_EN[dateObj.getMonth()]} ${dateObj.getFullYear()}`;
...
function renderMobileDateNavigator(selectedDate, { smooth = true } = {}) {
    ...
    for (let offset = -14; offset <= 14; offset++) {
        ...
        button.setAttribute(
            "aria-label",
            `${dateObj.getDate()} ${MONTHS_EN[dateObj.getMonth()]} ${dateObj.getFullYear()}`
        );

        weekday.className = "mobile-date-weekday";
        weekday.textContent = WEEKDAYS_EN_MOBILE[dateObj.getDay()];
